...
let desugar (p : top_prog) (*: lexpr*) =
  let rec desugar_prog (p: top_prog) : expr =
    match p with
    | [] -> Unit
    | (id, idlist, expr)::p_rest -> 
      Let (id, idlist, expr, desugar_prog (p_rest))
  in
  let rec desugar_let (expr) =
    match expr with
    | Let (id, [], ex1, ex2) ->
      Let (id, [], desugar_let ex1, desugar_let ex2)
    | Let (id, idlist, ex1, ex2) ->
      Let (id, [], Fun (idlist, desugar_let ex1), desugar_let (ex2))
    | Unit -> Unit
    | Num n -> Num n
    | Bool b -> Bool b
    | Var v -> Var v
    | Uop (u, ex) -> Uop (u,  desugar_let ex)
    | Bop (b, ex1, ex2) -> Bop (b, desugar_let ex1, desugar_let ex2)
    | App (ex1, ex2)-> App (desugar_let ex1, desugar_let ex2)
    | Ife (ex1, ex2, ex3) -> Ife (desugar_let ex1, desugar_let ex2, desugar_let ex3)
    | Trace (t) -> Trace (t)
    | _ -> assert false
  in
  let rec ds3 (expr) :lexpr =
    match expr with
    | Let (id, [], ex1, ex2) ->
      App (Fun (id , ds3 ex2), ds3 ex1)
    | Fun (idlist, ex) -> (
      match idlist with
      | h::[] -> Fun (h, ds3 ex)
      | h::t -> Fun (h, ds3 (Fun(t, ex)))
      | _ -> assert false
    )
    | Unit -> Unit
    | Num n -> Num n
    | Bool b -> Bool b
    | Var v -> Var v
    | Uop (u, ex) -> Uop (u,  ds3 ex)
    | Bop (b, ex1, ex2) -> Bop (b, ds3 ex1, ds3 ex2)
    | App (ex1, ex2)-> App (ds3 ex1, ds3 ex2)
    | Ife (ex1, ex2, ex3) -> Ife (ds3 ex1, ds3 ex2, ds3 ex3)
    | Trace (t) -> Trace (ds3 t)
    | _ -> assert false
  in ds3(desugar_let(desugar_prog p))
...
let rec translate (e : lexpr) : stack_prog =
  ...
  (*Add*)
    | Bop (Add, lex1, lex2) -> (translate lex2) @ (translate lex1) @ [Add]
  (*Sub*)
    | Bop (Sub , lex1, lex2) -> (translate lex2) @ (translate lex1) @ [Sub]
  (*Mul*)
    | Bop (Mul , lex1, lex2) -> (translate lex2) @ (translate lex1) @ [Mul]
  (*Div*)
    | Bop (Div , lex1, lex2) -> (translate lex2) @ (translate lex1) @ [Div]
  (*Neg*)
    | Uop (Neg , lex1) -> (translate lex1) @ [Push (Num 0); Sub]
  (*Not*)
    | Uop (Not , lex1) -> (translate lex1) @ [If([Push (Bool false)], [Push (Bool true)])]
  (*Less Than*)
    | Bop (Lt, lex1, lex2) -> (translate lex2) @ (translate lex1) @ [Lt]
  (*Less Than or Equal*)
    | Bop (Lte, lex1, lex2) -> (translate lex2) @ [Assign "lteb" ; Lookup "lteb"] @ (translate lex1) @ [Assign "lte" ; Lookup "lte"] @
      [Lt; If([Push (Bool true)], [Lookup "lte" ; Lookup "lteb"] @ [Lt; If([Push (Bool false)], [Push (Bool true)])])]
      (*Can use functions to not pollute environment*)
  (*Greater Than*)
  | Bop (Gt, lex1, lex2) -> (translate lex2) @ [Assign "gtb" ; Lookup "gtb"] @ (translate lex1) @ [Assign "gta" ; Lookup "gta"] @
      [Lt; If([Push (Bool false)], [Lookup "gta" ; Lookup "gtb"] @ [Lt; If([Push (Bool true)], [Push (Bool false)])])]
  (*Can use functions to not pollute environment*)
  (*Greater Than or Equal To*)
  | Bop (Gte, lex1, lex2) -> (translate lex2) @ (translate lex1) @
      [Lt; If([Push (Bool false)], [Push (Bool true)])]
      (*Can use functions to not pollute environment*)
  (*Equal to*)
  | Bop (Eq, lex1, lex2) -> (translate lex2) @ [Assign "eq" ; Lookup "eq"] @ (translate lex1) @ [Assign "eqb" ; Lookup "eqb"] @
      [Lt; If([Push (Bool false)], [Lookup "eqb" ; Lookup "eq"] @ [Lt; If([Push (Bool false)], [Push (Bool true)])])]
      (*Can use functions to not pollute environment*)
  (*Not Equal to*)
  | Bop (Neq, lex1, lex2) -> (translate lex2) @ [Assign "neq" ; Lookup "neq"] @ (translate lex1) @ [Assign "neqb" ; Lookup "neqb"] @
      [Lt; If([Push (Bool true)], [Lookup "neqb" ; Lookup "neq"] @ [Lt; If([Push (Bool true)], [Push (Bool false)])])]
      (*Can use functions to not pollute environment*)
  (*And*)
  | Bop (And, lex1, lex2) -> (translate lex1) @ [If((translate lex2) , [Push (Bool false)])]
  (*Or*)
  | Bop (Or, lex1, lex2) -> (translate lex1) @ [If([Push (Bool true)], (translate lex2))]
  (*If then else*)
  | Ife (cexpr, ifc, elsec) -> (translate cexpr) @ [If((translate ifc) , (translate elsec))]
  (*Fun Application*)
  | App (lex1, lex2) -> translate lex1 @ translate lex2 @ [Swap; Call]
  | Fun (id, lex) -> [Fun ("_", [Swap; Assign id] @ translate(lex) @ [Swap; Return])]

  let rec convert_name (id:ident) : ident =
    let go char : ident =
      match char with
      | '_' -> "CC"
      | c -> "A" ^ String.capitalize_ascii(Char.escaped c)
    in
    let rec go2 (clist: char list) =
      match clist with
      | [] -> ""
      | h::t -> go h ^ go2 t
    in
...
let rec serialize (p : stack_prog) : string = (* TODO *)
  let serialize_command command : string =
    match command with
    | Push c -> (
      match c with
    ...
    | Call -> "call"
    | Return -> "return"
    | If (ifc, elsec) -> "if " ^ serialize (ifc) ^ " else " ^ serialize (elsec) ^ " end"
    | Fun (id, prog) -> "fun " ^ convert_name(id) ^ " begin " ^ serialize (prog) ^ " end"
    | Assign (id) -> "assign " ^ convert_name(id)
    | Lookup (id) -> "lookup " ^ convert_name(id) 
  in
  match p with
  | [] -> ""
  | h::t -> serialize_command h ^ " " ^ serialize t
