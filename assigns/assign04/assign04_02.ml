...
*)
let rec path_check (g : 'a -> 'a -> bool) (iter: int) (len : int) (func: 'a -> 'a) (step : 'a) : 'a option =
  if iter >= len then Some step else
    match g step (func step) with
    | false -> None
    | true -> path_check g (iter+1) (len) (func) (func step)



let walk_check (g : 'a -> 'a -> bool) (len : int) (path: (('a -> 'a) * 'a)) =
  match path with
  | h, t -> (match path_check g 0 len h t with
    | None -> []
    | Some ans -> [ans]
    )
...
let walks
  ...
  let rec walks_check (g : 'a -> 'a -> bool) (len : int) (pstarts : (('a -> 'a) * 'a) list) : 'a list =
    match pstarts with
    | [] -> []
    | (h::t) -> (walk_check g len h) @ walks_check (g) (len) (t)
  in walks_check g len paths_starts
