...
(*
   ...
   return out
*)
let check_triple (tri: int * int * int) =
  match tri with
  | a, b , c -> (
    if a * a + b * b = c * c then
      true else false
...
let reverse_triple (tri: (int * int * int)) : (int * int * int) =
  match tri with
  | a, b, c -> c , b , a

let pythagorean_triples (n : int) (*: (int * int * int) list*) =
  for_loop (range 1 n) (fun i -> for_loop (range (i+1) n) (fun j -> for_loop (range (j+1) n) (fun k -> [i, j, k])))
  |> List.filter check_triple
